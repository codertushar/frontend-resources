...
import { Check, Star, Zap, BookOpen, Code, Sparkles, Shield, Tag, X, LucideIcon } from 'lucide-react';
...
export default function PricingContent() {
  ...
  const premiumArticles: number = articles.filter((a: Article) => a.premium).length;
  const freeArticles: number = totalArticles - premiumArticles;
  ...
  const handleApplyCoupon = async (): Promise<void> => {
    ...
    try {
      const result = await validateCoupon(couponCode.trim()) as CouponValidationResult;
      if (result.valid) {
        setAppliedCoupon({
          code: result.code!,
  ...
  const finalPrice: number | null = appliedCoupon && basePrice
  ...
  const handleCouponInputChange = (e: ChangeEvent<HTMLInputElement>): void => {
    setCouponCode(e.target.value.toUpperCase());
  };

  const handleCouponKeyDown = (e: KeyboardEvent<HTMLInputElement>): void => {
    if (e.key === 'Enter') {
      handleApplyCoupon();
    }
  };
