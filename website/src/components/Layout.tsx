import { useState, useRef, useEffect, useCallback, ReactNode, CSSProperties, MouseEvent, TouchEvent, ChangeEvent } from 'react';
...
const FOCUS_OPTIONS: number[] = [15, 25, 45, 60]; // minutes
const BREAK_OPTIONS: number[] = [5, 10, 15]; // minutes
...
const Layout = ({ children }: LayoutProps) => {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: globalThis.MouseEvent) => {
      if (megaMenuWrapperRef.current && !megaMenuWrapperRef.current.contains(e.target as Node)) {
  ...
  const handleMegaMenuClick = (e: MouseEvent<HTMLAnchorElement>) => {
    if (isMobile) {
      e.preventDefault();
